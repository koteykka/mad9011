...
1. **Understand Raster vs. Vector:** Grasp how Photoshop handles pixel-based images, and recognize why a raster logo can't be enlarged while a vector one can.
2. **Size of Screens:** Work in pixel dimensions and set up documents at the exact size a platform requires.
3. **Differentiate File Types:** Identify PSD, JPEG, PNG, GIF, and SVG, and choose the right one for a given asset.
4. **Navigate Photoshop Interface:** Familiarize with the layout, tools, panels, and menus, and learn workspace customization.
5. **Master Layers:** Understand layer creation, naming, grouping, management, and styling.
6. **Work with Artboards:** Understand the difference between a canvas and an artboard, and set up a document for multiple design sizes.
7. **Apply Nondestructive Editing:** earn the difference between destructive and nondestructive editing, keeping an editable working file separate from exported output using masks and Smart Objects.
8. **Build and Export a Banner:** Create a banner at a specified size and export it in the correct format.
...
Every image you work with is one of two kinds. Knowing which is which will save you a lot of grief.

Raster images are made of pixels — a fixed grid of tiny coloured squares. Each pixel holds data about colour and brightness. Photographs and screenshots are always raster. Photoshop is a raster program.

Vector images are made of mathematical paths — points, lines and curves. Nothing is stored as a grid, so the shape is redrawn at whatever size you need. Logos, icons and illustrations are usually vector.

## Why this matters immediately

Scaling a raster image down is safe. Scaling it up is not. There's no extra detail hiding in the file, so Photoshop invents pixels to fill the gaps and the result goes soft.

This is the most common problem you'll hit professionally. A client sends their logo as a 300px PNG, you need it across the top of a banner, and it can't be done — not because you lack skill, but because the pixels aren't there. What you needed was the vector file.

Rule of thumb: photographs are raster and should be sourced as large as possible. Logos and icons should be vector wherever you can get them.


### 2. Sizing Images for Screens

Everything in this course is made for screens, so there's only one number that matters: pixel dimensions.

Every platform publishes the exact pixel size it wants. You design at that size, and you export at that size.

When you create a new document, set the units to Pixels and enter the exact size. Don't design at a guess and resize later — start at the size you're delivering.

| **Format**                       | **Size**                                                               | **Where it Runs**                                                |
| ---------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------ |
| **Horizontal**         | 1200 × 628 px | 	Feed and link ads             |
| **Square**    | 	1080 × 1080 px                                              | 	Instagram feed |
| **Vertical** | 	1080 × 1920 px                                  | 	Stories and Reels     |
...
| ---------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------ |
| **PSD**         | Raster (layered) | 	Your working file — layers and masks preserved. The file you keep, not the file you send             |
| **JPEG**    | 	Raster                | 	Photographs. Lossy, so every re-save degrades it further |
| **PNG** | 	Raster                    | 	Banners, UI assets, anything needing crisp edges or transparency     |
| **GIF**    | 	Raster                  | 	Simple animation, maximum 256 colours |
| **SVG** | 	Vector                    | 		Icons and logos. Scales infinitely. Made in Illustrator, not Photoshop     |


### 4.The Photoshop Interface
- **Menu bar** — top of the screen. Everything lives here somewhere.
- **Options bar** — settings for whichever tool is active. It changes as you switch tools.
- **Toolbar** — left side. Hold-click any tool with a small triangle to reveal related tools underneath.
- **Panels** — right side. Layers, Properties, Colour and more. Add or remove them from the Window menu.
- **Canvas** — centre, where your work lives.
- **Contextual Task Bar** — a floating bar that appears near your work and offers the most likely next action.

## Workspaces

Photoshop ships with preset workspaces under Window → Workspace, each showing a different set of panels. Once you've arranged panels the way you like, save your own with Window → Workspace → New Workspace.

####  Canvas & Artboards

## The Canvas

The canvas is the base area where your design lives — a single piece of digital paper. Ideal for projects focused on one image: photo editing, illustration, digital painting.

## Artboards

Artboards let you hold multiple design areas inside one document. Like having several canvases in one file.

This is exactly what banner work needs. One campaign, three sizes, one document. Same images, same colours, same type, all in one place — so when something changes, you change it once.

You can name, rearrange and export artboards individually or all together.
...
Each layer is an independent stack of pixels you can reorder, hide, restyle or delete without touching anything else. This is what makes Photoshop editable rather than a one-shot paint program.

What you should be comfortable with this week:

- **Creating, duplicating, deleting and reordering layers.**
- **Naming layers meaningfully — "Layer 47 copy 3" helps nobody, including you next week.**
- **Grouping layers into folders (Cmd/Ctrl + G).**
- **Layer opacity and blend modes.**
- **Layer styles: drop shadow, stroke, gradient overlay.**
- **Locking layers to protect them from accidental edits.**

Pro Tip Naming and grouping isn't housekeeping, it's part of the work. A file someone else can open and understand is worth more than a beautiful one they can't. File organization is part of how your work is assessed in this course.
...
### 8. Building Your First Banner

In class we'll build one banner together from scratch. The steps:

- **New document** — set the size to 1200 × 628 px, units in Pixels, RGB colour.
- **Place your image** — File → Place Embedded.
- **Position and scale** it to fill the frame.
- **Add your headline** with the Type tool.
- **Add a supporting line** and a call to action.
- **Name every layer** as you create it.
- **Export — File → Export → Export As → PNG.**
